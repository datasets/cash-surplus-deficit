...
source = "https://api.worldbank.org/v2/en/indicator/GC.BAL.CASH.GD.ZS?downloadformat=csv"
script_dir = os.path.dirname(os.path.abspath(__file__))
cache = 'cache'
datapackage = 'datapackage.json'
current_year = datetime.now().year
outheadings = ['Country Name', 'Country Code', 'Year', 'Value']
data = 'data/cash-surp-def.csv'
archive = "archive/cash-surp-def.csv"
def search_files_in_cache():
    ...
    # Update data in archive folder
    with open('scripts/cache/'+filtered_files[0],'r') as firstfile, open(archive,'a') as secondfile: 
...
def process():
    ...
    shutil.rmtree(os.path.join(script_dir, cache))
